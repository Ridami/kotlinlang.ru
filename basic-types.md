...
### Знак подчеркивания при записи чисел ( с 1.1 )
Вы можете использовать подчеркивания, что бы сделать числовые константы более читаемыми:
``` kotlin
...
Отсутствие неявного преобразования редко бросается в глаза, поскольку тип определяется по контексту, а арифметические действия перегружаются для подходящих преобразований, например:
...
Операции над числами с плавающей точкой, описанные здесь, это:

* Проверка равенства: `a == b` и `a != b`
* Операции сравнения `a < b`, `a > b`, `a <=b`, `a >= b`
* Определение диапазона и проверка нахождения в диапазоне: `a..b`, `x in a..b`, `x !in a..b`

Когда известно что типы a и b являются статически определенными Float или Double либо их nullable-копиями (тип объявляется или определяется по конетексту, или является результатом [умного приведения](typecasts.html#smart-casts)), операции над числами и диапазонами их содержащими следуют IEEE 754 стандарту двоичной арифметики с плавающей точкой.

Для поддержки обобщенного использования операций тогда, когда операнды не типизированы, как числа с плавающей точкой (например `Any`, `Comparable<...>`, тип параметра), операции используют те реализации `equals` и `compareTo` для `Float` `Double`, 
которые отличаются от стандарта: 

* `NaN` считается равным себе.
* `NaN` считается больше, чем другие элементы, содержащие `POSITIVE_INFINITY` 
